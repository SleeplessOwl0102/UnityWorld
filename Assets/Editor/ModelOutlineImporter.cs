...
namespace JTRP.CustomAssetPostprocessor
{
    public class ModelOutlineImporter : AssetPostprocessor
    {
        ...
        public struct CollectNormalJob : IJobParallelFor
        {
            ...
            public NativeArray<UnsafeParallelHashMap<float3, float3>.ParallelWriter> result;

            public CollectNormalJob(NativeArray<float3> normals, NativeArray<float3> vertrx, NativeArray<UnsafeParallelHashMap<float3, float3>.ParallelWriter> result)
            {
        ...
        public struct BakeNormalJob : IJobParallelFor
        {
            ...
            [NativeDisableContainerSafetyRestriction]
            [ReadOnly] public NativeArray<UnsafeParallelHashMap<float3, float3>> result;
            [WriteOnly] public NativeArray<float2> uv8;

            public BakeNormalJob(NativeArray<float3> vertrx, NativeArray<float3> normals, NativeArray<float4> tangents, NativeArray<UnsafeParallelHashMap<float3, float3>> result, NativeArray<float2> uv8)
            {
                this.vertrx = vertrx;
                this.normals = normals;
        ...
        void ComputeSmoothedNormalByJob(Mesh smoothedMesh, Mesh originalMesh, int maxOverlapvertices = 50)
        {
            ...
            var result = new NativeArray<UnsafeParallelHashMap<float3, float3>>(maxOverlapvertices, Allocator.Persistent);
            var resultParallel = new NativeArray<UnsafeParallelHashMap<float3, float3>.ParallelWriter>(result.Length, Allocator.Persistent);
            // NormalBakeJob Data
            ...
            for (int i = 0; i < result.Length; i++)
            {
                result[i] = new UnsafeParallelHashMap<float3, float3>(svc, Allocator.Persistent);
